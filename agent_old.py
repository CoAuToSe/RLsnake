...
class AgentOld:
    ...
    def __init__(self):
        ...
        self.sequence_length = 5  # Par exemple, utiliser les 5 derniers états
    ...
    def get_state(self, game):
        ...
        if len(self.state_memory) < self.sequence_length:
            # Si la séquence n'est pas complète, on la remplit avec des zéros
    ...
    def train_step(self, états, actions, rewards, next_states, dones):
        return self.train_step_LSTM(états, actions, rewards, next_states, dones)
        
    def train_step_lin(self, états, actions, rewards, next_states, dones):
    ...
    def train_step_LSTM(self, états, actions, rewards, next_states, dones):
        ...
        pred_q_values, _ = self.model(états)
        ...
        for idx in range(len(dones)):
            Q_new = rewards[idx]
            if not dones[idx]:
                Q_new = rewards[idx] + self.gamma * torch.max(next_q_values[idx])
            target_q_values[idx][actions[idx].item()] = Q_new
    ...
    def get_action(self, state):
        return self.get_action_LSTM(state)
        
    def get_action_lin(self, état):
    ...
    def get_action_LSTM(self, état_sequence):
        ...
        if random.random() < self.epsilon:
            ...
            état_sequence = torch.tensor(np.array([état_sequence]), dtype=torch.float)  # Ajouter dimension batch
            état_sequence = état_sequence.to(self.model.lin.weight.device)
            with torch.no_grad():
                prediction, _ = self.model(état_sequence)
    ...
    def save(self, file_name='lstm_agent.pth'):
        checkpoint = {
    ...
    def load(self, file_name='lstm_agent.pth'):
